# ADR 0004 — Three Sync Run kinds: Bootstrap, Incremental, Catchup
...
A Sync Run executes a Worker against its Source and reconciles the result
into the Store. The simplest model has one shape — run, fetch, write.
...
- The **first** run on a fresh Store has no token. It must fetch a
...
- The token **expires** when the server's change log ages out. The server
  rejects the request and the Worker must recover without manual
  intervention.
...
bug: a fresh-window bootstrap *replaces* the Store content with whatever's
in the current window, losing rows for emails that are still in the Store
but older than the bootstrap horizon. For Cerebro's 30-day window, a
45-day-offline laptop would lose 15 days of historical rows on every
cursor expiry.

The PRD's Fastmail Worker explicitly avoids that bug.

## Decision

**Three Sync Run kinds, distinguished by trigger, not by status. The
`cerebro_sync_runs.status` enum stays binary (`succeeded` | `failed`);
the kind is implicit in which code path executed.**

### Bootstrap

- **Trigger.** `cerebro_sync_cursor` has no row for this Worker.
- **Behavior.** Fetch the configured window (30 days for Fastmail) from
  ...
  resulting state token to `cerebro_sync_cursor`.
...
- **Records.** `status=succeeded`,
  ...
  is the recovery signal — visible to anyone querying `cerebro_sync_runs`
...
- **The three kinds are implicit in code, not in schema** — the Worker
  decides which path ran based on cursor state and JMAP response.
  Someone querying `cerebro_sync_runs` cannot distinguish Bootstrap from
...
1. **A future audit/UI surface needs at-a-glance distinction between
   ...
   `cerebro_sync_runs` then; cheap forward-compatible schema change.
2. **A Source emerges that needs a fundamentally different recovery
   ...
   Define a new pattern for that Source's Worker; don't force-fit.
3. **Optimistic concurrency on writes** — if the Curator or Concierge
   eventually need to write to the same tables Workers maintain, the
   Sync Run state machine might need to coordinate with non-Worker
...
- [PRD #11 — Milestone 1, Fastmail vertical slice](https://github.com/rbrtbn/cerebro/issues/11) —
  Sync Run kinds pinned in Implementation Decisions; test scenarios
  enumerate Bootstrap, Incremental, and Catchup happy and failure paths.
- [`CONTEXT.md`](../../CONTEXT.md) — **Sync Run** definition.
