...
`CLAUDE.md` decided Pulse's database is **SQLite + Drizzle ORM (WAL mode)**,
...
the macOS Keychain on the home server that runs both projects' Connectors.
Both projects share that runtime. This raises a reasonable question of
whether Pulse should join the same Supabase project.
...
**Pulse stays on SQLite + Drizzle through Milestone 1.** Do not migrate to
Postgres in M1. Do not couple Pulse's storage to the photos Supabase
project yet.
...
- **Single-writer simplicity** for Connectors → Database. SQLite + WAL covers
  this for the foreseeable M1 throughput.
- **No external dependency** for Connectors and the Web App — the Database
...
- **Milestone 1 stays small**: the slice is `eversports-mcp → eversports
  Connector → gym_bookings table → Web App route`. Postgres adds nothing
  to that slice.

### Negative

- **Cross-project queries are awkward**: "emails about photos from Lisbon"
  needs to join photos.public + pulse.local — not possible while they're
  on different databases. Acceptable for M1 (no such queries exist).
- **Duplicate schema concepts**: if photos and Pulse both end up modeling
...
1. **Concurrent writers**: Reporter + Chat + multiple Connectors writing
   simultaneously. SQLite + WAL handles bursts well, but sustained
   multi-writer contention is where PG starts to dominate.
2. **pgvector for Chat RAG**: when the Chat wants embedding search
   over the Database, SQLite options exist (`sqlite-vss`, `sqlite-vec`) but
...
4. **Materialized views or streaming/CDC**: if Pulse's App needs
   Realtime subscriptions to push updates to the Web App as Connectors
...
`mail_*` / `pulse_*` prefixed tables to the same Postgres, switch
Drizzle's driver to `pg` (Drizzle supports this), and update the Connectors'
write path. Not free, but bounded.

## What this ADR does not decide

- **A future shared library between photos and Pulse**: deferred until
  real duplication surfaces.
- **Whether pulse joins photos in a monorepo**: separate concern; mixed
...
- For runtime host details (model servers, Keychain item names, what's
  ...
  if you have access. Pulse intentionally does not duplicate that
