# ADR 0005 — Metadata-only Database; bodies fetched on demand from the Source
...
PRD #11 stores only email metadata in `pulse_emails` — `id`,
...
Pulse metadata. No HTML bodies, no plain-text bodies, no attachments,
no recipient lists.
...
for the longer arc of Pulse:

- **M1's `/inbox` view** only renders metadata fields, so storage matches
  view-shape exactly.
- **The future Reporter** — the agent that will produce Digests, classify
...
- **The future Chat** — the on-demand chat layer — will also want to
  ask "what did Mira say in her last email?" which needs body content.

The naive read of "Reporter + Chat need bodies eventually" is "store
...
**The Database mirrors only the metadata fields `/inbox` renders. Bodies,
attachments, and recipient lists are fetched on demand from the Source
when future features need them.**

The fetch-on-demand path uses the same `packages/jmap` client the Connector
uses (per ADR 0003's "one place owns Source semantics" rule). When the
Reporter needs the body of `pulse_emails.id = X`, it calls
`packages/connectors/fastmail.fetchBody(id)` — a Connector method, not a
direct JMAP call from the Reporter process. The credentials reach
...
shape.** `fetchBody` doesn't exist in M1 because the Reporter doesn't
exist in M1.
...
- **Privacy footprint is minimal.** Email metadata is sensitive; email
  ...
  Database reduces the local-disk-leak blast radius. If `data/pulse.db`
...
- **Caching invalidation is the Source's problem, not ours.** When a
  ...
  to a thread, Fastmail knows. Pulse doesn't need to track or
  invalidate cached body content.
- **Pulse doesn't become a backup of Fastmail.** It's an aggregator
  over Fastmail. The Source remains authoritative.

### Negative

- **Body access requires network round-trips** when the Reporter or
  Chat wants them. JMAP's `Email/get` for one email is fast (~50ms)
  but adds latency the "stored locally" path would skip.
- **Offline operation is limited.** With no bodies in the Database, an
...
- **Multiple round-trips for batch operations.** Categorizing 100
  ...
  call). The Connector's batching layer needs to be efficient.
- **The Source must stay reachable.** Fastmail downtime affects body
  access. Metadata reads from the Database still work; body-dependent
  features don't.
...
1. **The Reporter's categorization quality demands body content
   *and* the network round-trip cost or batch latency becomes a real
   ...
   bite.
2. **Offline use becomes a Milestone goal.** If Pulse needs to function
...
`body_html` (nullable) columns to `pulse_emails`; backfill via a one-off
Sync-Run-shaped script. Categorization writes into the future
`pulse_email_categories` table either way; the categories themselves
don't depend on bodies-being-stored.

## What this ADR does not decide

- **Categorization mechanism** (rules vs AI vs hybrid). Reporter
  territory; out of M1 scope.
- **The shape of the future `pulse_email_categories` table.** Sketched
  in PRD #11's "Longer arc" note; pinned when the Reporter is real.
- **Chat's body-access pattern** — direct via `packages/jmap` or
  via a Connector method like `fetchBody(id)`. ADR 0003's "one place owns
  Source semantics" implies the Connector; the exact method shape waits
  for the Chat.
- **Caching of fetched bodies.** A short-lived in-memory cache in the
  Reporter process to avoid re-fetching the same email twice in one
...
- [PRD #11 — Milestone 1, Fastmail vertical slice](https://github.com/rbrtbn/pulse/issues/11) —
  storage scope pinned; "Longer arc — Pulse as smart inbox" note
  references this ADR.
- [ADR 0003 — App-driven writes round-trip through a Connector](./0003-interface-writes-via-worker.md) —
  the "one place owns Source semantics" rule that shapes how the future
  body-fetch method will be exposed.
- [`CONTEXT.md`](../../CONTEXT.md) — **Reporter** and **Chat**
  definitions.
