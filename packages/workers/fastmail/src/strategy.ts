...
import { getEmailIdsSince, StoreDb } from "@cerebro/store";
 ...
 * these and records them as a failed `SyncRun` row with the tag's name
...
/**
 * A Sync Run strategy. M1.1 shipped Bootstrap; M1.2 adds Incremental
 * and Catchup. All strategies share this shape so the envelope can
 * swap between them without knowing which kind ran.
 *
 * Strategies can **read** from the Store (Catchup's local-ID diff
 * needs `getEmailIdsSince`); they don't **write** — that's the
 * envelope's job. A `StoreError` from a strategy's read escapes
 * through the envelope's residual the same way `StoreError` from the
...
export type Strategy = Effect.Effect<
  StrategyResult,
  StrategyError | StoreError,
  FastmailJmap | StoreDb
>;
 ...
 * still propagate up to the envelope as failed Sync Runs.
 *
 ...
 * in the Store until the next Catchup reconciles. ADR 0004 records that
 * trade-off.
 */
 ...
 * upstream window, diff against the Store's IDs in the same window, fetch
